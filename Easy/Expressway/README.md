...
Since it is leaking information such as VID(Vendor ID).
Let's run an aggresive mode scan and see if it leaks any sensitive information
...
```bash
sudo ike-scan -M -A 10.10.11.87 -n ike@expressyway.htb -Pout.txt
```
Now we will have our PSK in out.txt.

Let's run an offilne dictionary on it.We can use hashcat for psk-crack.
...
So I check which binary was running
...
After some searching regarding proxy group, I found that it may have acces to senstie log files at /etc/squid or /etc/haproxy. So i just check for all log files.
...
This log revelead an internal-only hostname.
